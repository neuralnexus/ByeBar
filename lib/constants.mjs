...
export const FEATURE_KEYS = ['genericBlocking', 'cookieDecline', 'tosAccept'];

export const GLOBAL_BOOLEAN_KEYS = ['enabled', ...FEATURE_KEYS];
...
export const BLOOMBERG_HOST_PATTERNS = [/(?:^|\.)bloomberg\.(?:com|co\.jp|tv)$/i];
...
export const OVERLAY_PROMOTION_RE =
  /\b(?:join|subscribe\s+to|sign\s*up\s+for)\s+(?:(?:our|the|a)\s+)?(?:newsletter|mailing\s+list|email\s+list|updates)\b|\b(?:newsletter|email)\s+sign[-\s]?up\b|\bjoin\s+(?:our|the)\s+community\b|\bsign\s*up\s+to\s+(?:get|receive)\b.{0,80}\b\d{1,2}%\s+off\b|\b(?:get|save|unlock)\s+\d{1,2}%\s+off\s+(?:your\s+first|when\s+you|by\s+|today)\b|\b\d{1,2}%\s+off\s+your\s+first\s+(?:order|purchase)\b|\bexclusive\s+(?:offers|discounts)\s+(?:by\s+email|in\s+your\s+inbox)\b/i;

export const OVERLAY_MANAGEMENT_RE =
  /\bmanage (?:your )?subscription\b|\bcancel (?:your )?subscription\b|\bsubscription (?:settings|billing|plan|status)\b|\b(?:sign|log)\s*in\b|\bauthentication\b|\bverify\b|\bverification\b|\baccount\b|\bprofile\b|\bsettings\b|\bpreferences\b|\bcheckout\b|\bcart\b|\bbilling\b|\bpayment\b|\bpassword\b|\bcontinue\s+(?:to|with)\b|\bapply\s+(?:a\s+)?(?:discount|promo)\s+code\b/i;
